...
We have validated some results of the original paper despite some differences that may be due to the initialization. By adding an experiment, we contradict an assertion of the paper concerning the robustness of the model to the size of the embeddings. An additional result, on the impact of our hyperparameter \texttt{split}, shows its importance on the final performance.

%We have validated some of the authors' claims through the experiments given in the original article and invalidated other claims through some additional experiments of our own. Overall, we found that many experiments gave results with marginal differences. The additional experiments had to verify that for any size of embedding, the performance did not change.
...
\end{itemize}
By organizing the work in this way, it allows to have a modular, easily reusable and understandable code. To run the model, it's necessary to create an environment with the required packages and run one of the following python scripts in order to lauch the trainning/evaluation of the JODIE model on one of avaialble dataset:
\begin{itemize}
...
\end{itemize}
More informations are available in README file in the repositoritory of our code\footnote{\url{https://github.com/ComplexNetTSP/JODIE}}.
