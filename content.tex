...
\textcolor{red}{For the figure on left, the larger the training set is the more the model predicted classes are wrong. This can be explained by the big imbalance in the dataset. There is only few examples of state change in the datasets. Learning on a small training set will lead the model to only characterize the majority class. The model will then over-predict this class. Knowing that the test set will contain a majority of this class, the accuracy will be high despite some few examples of the minority class that is not enough seen during the learning task to be correctly predicted. When the training set is getting larger, the model start also characterizing the minority class which is now more represented but still not enough. This causes more confusion during the prediction which leads to lower accuracy. The figure on right shows us that larger the training set is, the better is the model in the predicting the future interactions. This describe the classical behavior of models, the more data the see the higher the results are. If we were to compare the curves obtained by the authors of the paper and ours, we could say that the general shapes are similar.}
