...
\section*{Conclusion}
We are able to successfully replicate the original code provided by the authors and reproduce the results obtained in original JODIE publication. 

Even if our reproduction more or less match the result provided in the orginal study, we found some sligh difference in the result compare with the orginal paper. This can be explained by a variation due to the initialization. However, for LastFM dataset our result clearly outperform those claim in the original study, we are suspecting some reporting error in the original study.

About the impact of the embedding size on the overall result, despict the claim made by the author, we show that it has a slight an impact with respect of the metric used. 

Finally, the split has a strong impact on the overall performance, This hyper-parameter must be adjusted taking into account a complexity-performance trade-off.\\
