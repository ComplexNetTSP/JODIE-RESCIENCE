...
In eq. \ref{eq:1} we compute the index of the batch $B_{idx}$ where the interaction $\mathcal{S}_r \in \mathcal{S}$ wille be put in. Let $\text{maxBatch}(e, \,r-1)$ be the function that gives the index of the batch at time $r-1$ containing the entity $e$. Moreover, in each batch there is an edge with two nodes of the graph that are totally independent from the other edges in the same batch. By ..... we are guaranteed to make batches indepênadant of each other (parallelizable) and the events ordered.
