...
In this work, the article Predicting Dynamic Embedding Trajectory in Temporal Interaction Networks~\cite{kumar2019predicting} is evaluated through a replication study. Replication results examine whether the claims made by \textcolor{green}{the} authors are valid. Our goal is to replicate the experiments and achieve the same results as the authors.
...
%After replication of the model and we validated most of the results found in the original paper, we have found only marginal difference between the replication and the original study. 
\textcolor{green}{After replicating the model and validating most of the results found in the original paper, we found only a marginal difference between the replication and the original study. However, our extensive experiment regarding the robustness of embedding size led us to slightly different conclusions than those of the authors. In addition, we extended the original study by conducting new experiments on the impact of the \texttt{split} on the model's performance. As a result, we found that the \texttt{split} has a real impact on the overall performance. 
}
%However, our extended experimentation about the robustness of embeddings size has lead us toward slightly different conclusion than those define in the original study. 
%Additionally, we extended the original study by making some new experimentation about the impact of the \texttt{split} on the performance of the model, we have found that the \texttt{split}  has a real impact on the overall performance. 
...
\textcolor{green}{Although the code is available, some technical aspects hindered our complete understanding of the model. We solved them by investigating the code provided by the authors.}
...
\textcolor{green}{We contacted the authors, asking them what aggregation method they used to meet the RNN gate input constraints.}
...
Our code \textcolor{green}{and trained models are publicly available at \url{https://github.com/ComplexNetTSP/JODIE.}} %is available at the following address: \url{https://github.com/ComplexNetTSP/JODIE}.
We want to point out that we have used the \textit{tanh} function as the activation function at the output of the RNNs (see Figure \ref{Pipeline}) to build our model in accordance with the original code but in contrast to what the authors wrote in the original paper.
% We want to highlight the fact that we used the \textit{tanh} function  as %the activation function at the output of the RNNs (see fig.~\ref{Pipeline}) to build our model, in accordance with %to the authors' original code but contrary to what's written in the original paper. 
...
\begin{itemize}
    ...
    includes data preprocessing and the t-batch code.
...
Organizing the code this way makes it meaningful and easier to reuse and considerably reduces the effort for code reviews and tests. 
%To run the model, it's necessary to create an environment with the required packages and run one of the following python scripts in order to lauch the training/evaluation of the JODIE model on one of available dataset:
Before running the model, it is necessary to create an environment with the required packages and run one of the following python scripts to launch the training/evaluation of the JODIE model on one of the available datasets:
\begin{itemize}
...
\end{itemize}
\textcolor{green}{Further information is available in the GitHub README file.\footnote{\url{https://github.com/ComplexNetTSP/JODIE}}}
%More informations are available in README file in the repository of our code\footnote{\url{https://github.com/ComplexNetTSP/JODIE}}.

\subsection{Model description}%descriptions}

\subsubsection{Update operator}
The update operator takes the form of two mutually recursive neural networks as depicted %shown 
in Figure \ref{recursive RNNs}. %The RNN$_U$ is used to update the dynamic user embeddings and the RNN$_I$ updates the dynamic item embeddings.
RNN$_U$ updates users' dynamic embeddings, whereas RNN$_I$ updates those of the items. The embeddings are thus considered as the hidden states of the classical RNNs.%In this case, the embeddings are considered as the hidden state of the classical RNNs respectively. 

At time $t=0$, the embeddings (noted $u_0$ and $i_0$) are initialized randomly according to a uniform distribution $\mathcal{U}[0;1[$ followed by a normalization. %,  and are noted $u_0$ and $i_0$. 
In addition to the embeddings at time $t=0$, the RNNs take as input $\Delta$, the time elapsed between an entity and the previous interaction, and $f$ a feature vector which characterizes the link of the interaction.
%As input, there is the embeddings at time $t=0$ but also $\Delta$ the time elapsed between an entity and the previous interaction and $f$ a feature vector which characterizes the link of the interaction. 
As we can observe, the %A
specificity of these RNNs is that they take four inputs instead of the standard two. %as usual. 
In order to narrow down to just two entries,%inputs, 
the embeddings of the other entity $\Delta$ and $f$ are concatenated.
%we concatenate the embedding of the other entity, $\Delta$ and $f$. 
In Figure \ref{recursive RNNs},
\textcolor{red}{[.,.] is used to denote the concatenation. More formally, the embeddings are updated with the following formulas:}%we denote the concatenation by [.,.]. More formally, we use the following formulas to update the embeddings:
$$
...
$$
where the matrices $W_1^e, ..., W_4^e$ are the parameters of the RNN$_e$\textcolor{green}{?} and $\sigma$ an activation function (here hyperbolic tangent). \textcolor{red}{$u^-$ and $i^-$ \textcolor{green}{represent} %represents 
the embeddings before updating and $u^+$ and $i^+$ \textcolor{green}{represent} %represents 
the embeddings after updating. Once this step is finished, the \textcolor{green}{next} %second 
operation, \textcolor{green}{called the projection operation,} can be \textcolor{green}{launched} %used, %it's called the projection operation.} %We note by $u^-$ and $i^-$ the embeddings before update and by $u^+$ and $i^+$ the embeddings after update. Once this step is finished, we can proceed to the second operation, the projection operation.
...
The first term minimizes the error of the predicted embedding. The next \textcolor{green}{two}  %twos 
terms regulate the loss function, and \textcolor{green}{ensure  that  } %allow 
the dynamic embeddings of users and items %to 
\textcolor{green}{do} not vary too much. $\lambda_U$ and $\lambda_I$ are the regularization terms that penalize the two terms. In the original paper, the authors fixed the values of $\lambda_U$ and $\lambda_I$ as $\lambda_U = \lambda_I = 1$, without much justification. \textcolor{geen}{But it is unclear whether this is the best value or } %is not clear if this is the best value or 
if another value would be more appropriate. \\
...
%The prediction and evaluation stage are just as special as the learning stage. In the evaluation, there is still a "learning" \textcolor{green}{stage} because during the predictions, the model will update itself respecting the time of the periods defined by \textcolor{green}{the} \texttt{split} \textcolor{green}{algorithm??? hyper-parameter???}. This evaluation is done thanks to the function \textbf{evaluate} of the file \textbf{evaluate.py}. During the evaluation, we do not apply the \texttt{t-batch} algorithm on the remaining interactions, but we will treat them one after the other (Figure \ref{pipeline_t-batch} not t-batch but interaction just before the back-propagation).
