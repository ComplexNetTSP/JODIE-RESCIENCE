...
\begin{enumerate}
    ...
    \item To begin, we will define two terms: \texttt{split} and period. \texttt{Split} is an integer used to separate time into several intervals. Period is a time interval. The JODIE model has a particularity in the way the \texttt{t-batch} is formed. Usualy, to batches, we split teh observations the dataset within a givent number of batches. But when we have a dataset ha s temporal dependencies the operation can not be done the same way. Tha why apply the \texttt{t-batch} algorithm is applied. But, Instead of applying directly the algorithm on the whole dataset, the authors decided to apply independently the \texttt{t-batch} algorithm on each subset of the data as described int the figure \ref{pipeline_t-batch}. From the temporal bipartite graph, we will separate the edges and classify them in an increasing temporal order. Then, we will take the time interval where the interactions occur and we will divide this time by a hyper-parameter called \texttt{split} (in the original papaer \texttt{split} is set to to 500). Finally, we will apply the \texttt{t-batch} algorithm on each period. Once all the t-batches are processed, we can back-propagate the error for one period.
