...
We reimplement the method of Kumar et al.\supercite{kumar2019predicting} for recommender systems using their new algorithm that allows batch learning on temporal data, improves their codes, and extends their work. Our improvements consist in using the last versions of Python and  Pytorch, and having a more readable code. Our extensions include finding the best hyperparameters using the \textbf{Ray-Tune} library. The codes are available on gitHub.
...
The original paper describes recommender systems as a temporal bipartite graph where nodes represent users and products and an edge represents an interaction between a user and an item. 
...
%In addition to reproducing the results presented in the paper, we extend the values for the choice of embedding size to 8, 16, 32, 64 and 128.
We perform new experiment\amel{s} to demonstrate the impact \amel{of} the frequency at which the batches are created, i.e the split.
\section*{Methodology}
...
\begin{table}[H]
    ...
    & JODIE & \multicolumn{3}{c}{Replications} \\
    \cmidrule{2}
    & 128 & 32 & 128 & 256 \\
    \midrule
    Reddit &&&&\\
...
\begin{figure}[H]
	\centering
    \subfigure[A]{\includegraphics[trim = 0cm 0cm 0cm 0cm, clip,width=0.46\textwidth]{image/wiki.pdf}} 
    \subfigure[B]{\includegraphics[trim = 0cm 0cm 0cm 0cm, clip,width=0.46\textwidth]{image/wiki_state.pdf}}
