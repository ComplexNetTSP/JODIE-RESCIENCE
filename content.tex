...
\begin{itemize}
    ...
    \item \textbf{More accurate state change performance}. The authors claim that JODIE outperforms five models regarding the user state change over all the selected datasets with an average improvement of 12.63\% for the Area Under the Curve (AUC) metric.
    ...
    \item \textbf{Robustness of the model to the embedding size}. The size of the embeddings does not have much impact on the performance of JODIE.
\end{itemize}
...
This operation consists in projecting the user's embedding to a future time. Indeed, it is assumed that user's interest changes with time, for example in winter and in summer, hence the need to model this variation as a linear projection. To make the explanation more clear, the authors present a figure %\ref{Projection} 
to better understand (cf figure 3 in~\cite{kumar2019predicting}).\\
...
% dire que le t-batch est un point clé de la méthode qui permet d'accélérer le temps d'apprentissage
% Difficile de compréhension dans l'article original (quelques lignes) et la compréhension dans le code a été difficile également. Dire les différentes façons de faire le t-batch (au début pour nous, première epoch pour eux).
...
The goal of the \texttt{t-batch} algorithm is to be able to separate temporal data into batches. 

This algorithm is a key point of the method that allows to speed up the learning time considerably. The \texttt{t-batch} algorithm is explained in the original article by a few lines. Concerning the code, the algorithm was difficult to identify in the learning loop. As a result, it took us some time to understand this algorithm. To facilitate the understanding of the \texttt{t-batch} we propose algorithm \ref{t-batch} and explain its principle.
